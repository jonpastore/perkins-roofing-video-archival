...
- [x] 🔴 **PROFIT / MARGIN IS READABLE BY `quoting_view` AND `estimating_view`** — closed
      2026-08-14. `api.routes.estimator._public_estimate` strips profit/margin/commission
      (and internal calc_lines) on every read unless the caller holds `estimating_manage`.
      Persist is unchanged so a manager can still reconcile. `/estimator/rates` still serves
      config fields to `estimating_view` — that is a rate table, not a job's margin.
