...
### The related-links backfill is DONE — `fix.txt` was stale, do not re-run it

`fix.txt` sat untracked at repo root saying "Backfill not applied. Dry run only: 474 scanned,
465 would change … 273 SCHEDULED will publish broken." **That was true when written and false
three hours later.** `955e9a5` (2026-08-12 11:29) applied it and verified on the live site:
**465 articles repaired in the database, 204 published bodies and 473 total bodies repushed,
0 failures**, publish dates preserved, scheduled posts still draft. The file has been deleted.

Recorded because the *reason* it took two passes is a trap the next reader can fall into:

- **`--repush` alone is not enough for scheduled articles.** It targets `status == 'published'`
  only. The script's own header docstring still claims `promote_job` "publishes them from THIS
  content" — **that is wrong**; `jobs/promote_job.py:112` calls `wordpress.update_status(...)`
  and only flips the status, never sending the body. So corrected DB rows + stale WordPress
  bodies meant draining the queue would have published the exact defect on all 262 of them.
  **`--repush-scheduled` is the flag that fixes those**, pushing with `status=None` so
  WordPress's own status is untouched (sending `"future"` without a `date` publishes it
  immediately — see `adapters.wordpress.update`).
- The first apply looked like it failed and had not: one DB session held open across ~15 min of
  HTTP pushes tripped prod's `idle_in_transaction_session_timeout`, giving **473 pushed, 0
  failed, exit 1**. It now detaches before the network phase.

The underlying bug, for context: three passes appended related links under non-equivalent
guards, and `_relativize_internal_links` erased the `BASE_URL` string `_append_service_links`
used to detect its own output, so each generate→repair→relativize cycle added a block. All three
now route through `core.article_repair.merge_related_block`. The old gate missed it because
`_RELATED_BLOCK_RE.search` only ever examined the *first* block.

On Wendy's five items: **she was right and understated it** — 183/183 generated articles carried
2–4 related-links blocks (she reported 3). Her other four were clean 183/183 (legacy YouTube
URL, in-content TOC, unlinked "Learn more", borderless tables).
...
The related-links backfill is **not** on this list — it was applied and verified on 2026-08-12
(§7). The untracked `fix.txt` that said otherwise was stale by three hours.
