...
def _ollama(path, payload, timeout=300):
    # think=False on every generate: the dev model (qwen3.6) is a HYBRID REASONING model where
    # the old default (mistral) was not, so without this it burns a few hundred reasoning tokens
    # before every answer — 126 eval tokens to say "OK" vs 2. Ollama returns that reasoning in a
    # separate `thinking` field, so the <think> regex strips in chat()/OllamaLLM never catch it.
    # Set here rather than per-caller: both app.llm.chat and adapters.llm.OllamaLLM route through
    # this helper, and only one of them had the guard.
